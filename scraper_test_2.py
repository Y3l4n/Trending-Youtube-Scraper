...
# Column headers
header = ["title", "publishedAt", "channelTitle", "category", "tags", "duration", "view_count", "likes",
          "dislikes", "comment_count", "subscriber_count", "engagement_rate", "description", "region_restriction"]
...
def get_videos(api_key, items):
    ...
    for video in items:
        ...
        line = features + [prepare_feature(category)] + [prepare_feature(x) for x in [
            tags, duration, view_count, likes, 0,  # Dislikes are no longer available
            comment_count, subscriber_count, engagement_rate, description, region_restriction
        ]]
